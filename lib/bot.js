  ...
  attachListeners() {
    ...
    if (this.ircStatusNotices.join) {
      this.ircClient.on('join', (channel, nick) => {
        if (nick !== this.nickname) {
          this.sendToSlack(false, channel, `*${nick}* has joined the IRC channel`);
        }
      });
    }
    ...
    if (this.ircStatusNotices.leave) {
      this.ircClient.on('part', (channel, nick) => {
        this.sendToSlack(false, channel, `*${nick}* has left the IRC channel`);
      });

      this.ircClient.on('quit', (nick, reason, channels) => {
        channels.forEach(channel => {
          this.sendToSlack(false, channel, `*${nick}* has quit the IRC channel`);
        });
      });
    }
  }
