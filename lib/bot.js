...
class Bot {
  ...
  attachListeners() {
    ...
    this.ircClient.on('whois', (info) => {
      const date = new Date(Date.now() - (info.idle || 0) * 1000);
      this.sendToSlack(false, this.nickname, `
        WHOIS for *${info.nick}*
        (_${info.user}@${info.host}_): _${info.realname}_
        _${info.server}_ :_${info.serverinfo}_
        ${info.accountinfo} _${info.account}_
        idle since _${date.toDateString()}, ${date.toLocaleTimeString()}_
      `);
    });
    ...
    if (this.ircStatusNotices.modes) {
      this.ircClient.on('+mode', (channel, by, mode, argument) => {
        this.sendToSlack(false, channel, `*${by}* sets mode *+${mode}* on _${argument || channel}_`);
      });
      this.ircClient.on('-mode', (channel, by, mode, argument) => {
        this.sendToSlack(false, channel, `*${by}* sets mode *-${mode}* on _${argument || channel}_`);
      });
  ...
  sendDMToIRC(message, channel) {
    const text = this.parseText(message.getBody());
    ...
    if (cmdMatch) {
      ...
      // reset the PM timestamp, so you don't accidentally message them after a command
      this.lastPM.timestamp = 0;
      return;
    }
  ...
  sendToIRC(message) {
    ...
    if (channel.is_im) {
      this.sendDMToIRC(message, channel);
      return;
    }
